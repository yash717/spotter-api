...
USERS_DATA = [
    ...
    {
        "email": "fleet1@swift.com",
        "first": "Lisa",
        "last": "Park",
        "role": MemberRole.FLEET_MANAGER,
        "org": 0,
    },
    {
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if User.objects.filter(email="admin@swift.com").exists():
            self.stdout.write(self.style.WARNING("Seed data already exists. Skipping."))
            return

        self.stdout.write("Creating PLATFORM_ADMIN (superadmin)...")
        ...
        orgs = [
            Organization.objects.create(
                ...
                mc_number="MC654321",
                primary_contact_email="admin@swift.com",
                address="100 Fleet Way, Dallas TX 75201",
                phone="214-555-0100",
            ),
            Organization.objects.create(
                name="Apex Freight",
                ...
                mc_number="MC210987",
                primary_contact_email="admin@apex.com",
                address="200 Carrier Blvd, Atlanta GA 30301",
                phone="404-555-0200",
            ),
        ]
        ...
        for i in range(6):
            ...
            trip = Trip.objects.create(
                ...
                status=random.choice(
                    [TripStatus.DRAFT, TripStatus.ASSIGNED, TripStatus.ACTIVE, TripStatus.COMPLETED]
                ),
                input_current_address=c1[0],
                input_pickup_address=c2[0],
                input_dropoff_address=c3[0],
                input_current_lat=Decimal(str(c1[1])),
                input_current_lng=Decimal(str(c1[2])),
                input_pickup_lat=Decimal(str(c2[1])),
                input_pickup_lng=Decimal(str(c2[2])),
                input_dropoff_lat=Decimal(str(c3[1])),
                input_dropoff_lng=Decimal(str(c3[2])),
            ...
            Stop.objects.create(
                ...
                sequence_number=1,
                location_lat=Decimal(str(c1[1])),
                location_lng=Decimal(str(c1[2])),
                address_text=c1[0],
                duration_minutes=0,
            )
            Stop.objects.create(
                ...
                type=StopType.FUEL,
                sequence_number=2,
                location_lat=Decimal(str((c1[1] + c2[1]) / 2)),
                location_lng=Decimal(str((c1[2] + c2[2]) / 2)),
                address_text="Fuel Stop",
                duration_minutes=30,
            )
            Stop.objects.create(
                trip=trip,
                type=StopType.FINAL_DROPOFF,
                sequence_number=3,
                location_lat=Decimal(str(c3[1])),
                location_lng=Decimal(str(c3[2])),
                address_text=c3[0],
                duration_minutes=0,
            )
            ...
            DutyStatusSegment.objects.create(
                ...
                start_time=datetime.now(UTC).replace(hour=6, minute=0),
                end_time=datetime.now(UTC).replace(hour=17, minute=0),
                status=DutyStatus.DRIVING,
                duration_minutes=int(min(driving_hrs, 11) * 60),
                segment_label=f"Driving {c1[0]} to {c3[0]}",
                sequence_in_day=1,
            )
        ...
        for i, (email, role) in enumerate(
            [
                ...
                ("pending3@trucking.com", MemberRole.FLEET_MANAGER),
            ]
        ):
            Invitation.objects.create(
                ...
                email=email,
                role=role,
                token_hash=f"seed_placeholder_{i}",
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                f"  PLATFORM_ADMIN (superadmin): superadmin@spotter.ai / {SEED_PASSWORD}\n"
            )
        )
